...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    # For this example, we will mock the data
    forecast = {
        "location": location, 
        "forecast_days": days,
        "current": {
    ...
    for hour in range(24):
        forecast["hourly"].append({
            ...
            "temperature_c": round(random.uniform(15, 30), 1),
            "condition": random.choice(["sunny", "partly_cloudy", "cloudy"]),
            "solar_irradiance": round(random.uniform(0, 1000), 1),  # in W/m²
            "humidity": random.randint(30, 80),
            "wind_speed": round(random.uniform(0, 15), 1)
        })
    return forecast
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    base_price = 0.15  # base price per kWh in USD
    for hour in range(24):
        if 6 <= hour < 22:
            rate = round(base_price * random.uniform(1.2, 1.5), 3)  # peak hours
            period = "peak"
            demand_charge = round(random.uniform(0.05, 0.10), 3)
        else:
            rate = round(base_price * random.uniform(0.7, 1.0), 3)  # off-peak hours
            period = "off-peak"
            demand_charge = 0.0
        
        electricity_prices["hourly_rates"].append({
            "hour": hour,
            "rate": rate,
            "period": period,
