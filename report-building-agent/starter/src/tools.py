...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator_func(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression to evaluate (e.g., "2 + 3 * 4", "(10 - 5) / 2")
            
        Returns:
            The calculated result as a formatted string, or an error message if invalid.
        """
        ...
        if not safe_pattern.match(expression.replace('**', '^').replace(' ', '')):
            error_msg = f"Error: Invalid expression '{expression}'. Only basic math operations (+, -, *, /, **, (), numbers) are allowed."
            logger.log_tool_use("calculator", {"expression": expression}, error_msg)
            return error_msg
        ...
        try:
            ...
            logger.log_tool_use("calculator", {"expression": expression}, formatted_result)
